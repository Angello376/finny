...
const financeRepository = {
  async list(): Promise<FinanceCard[]> {
    try {
      const cards = await withFinanceStore<FinanceCard[]>("readonly", (store) =>
        store.getAll(),
      );
      return cards.sort(sortCardsByUpdatedAt);
    } catch {
      return localStorageRepository.list().sort(sortCardsByUpdatedAt);
    }
  },
  async save(card: FinanceCard, currentCards: FinanceCard[]) {
    try {
      await withFinanceStore<IDBValidKey>("readwrite", (store) => store.put(card));
    } catch {
      const nextCards = [
        card,
        ...currentCards.filter((currentCard) => currentCard.id !== card.id),
      ].sort(sortCardsByUpdatedAt);
      localStorageRepository.save(nextCards);
    }
  },
  async remove(cardId: string, currentCards: FinanceCard[]) {
    try {
      await withFinanceStore<undefined>("readwrite", (store) => store.delete(cardId));
    } catch {
      localStorageRepository.save(
        currentCards.filter((currentCard) => currentCard.id !== cardId),
      );
    }
  },
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    financeRepository.list().then((storedCards) => {
  ...
  async function persistDraft(image?: GeneratedImage) {
    ...
    await financeRepository.save(nextCard, nextCards);
    setMessages([]);
    return nextCard;
  }
  ...
  async function deleteCurrentCard() {
    ...
    setCards(nextCards);
    await financeRepository.remove(draft.id, nextCards);
    setDraft(null);
    setScreen("home");
